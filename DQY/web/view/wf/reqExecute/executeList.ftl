...
<script type="text/javascript">
    function pagerAction(start, rows) {
        var searchUrl = '/wf/reqExecute!executeList.dhtml';
            ...
            <#if reqExecuteList?exists&&reqExecuteList?size gt 0>
                ...
                <tr <#if (reqExecute_index+1)%2!=0>class="oddBgColor"</#if>>
                    ...
                    <td style="text-align: center;">
                        <#if reqExecute.executeRead==0>
                        <i class="icon-eye-close"></i>未查看
                        <#elseif reqExecute.executeRead==1>
                            <i class="icon-eye-open"></i>未查看
                        </#if>
                    </td>
                    <td style="text-align: center;">${(reqExecute.receiveDate)?string("yyyy-MM-dd HH:mm")}</td>
                    <td style="text-align: center;">
                        <span style="cursor: pointer;" class="taskReq"  uid="${reqExecute.id?c}"><i class="icon-hand-down"></i>处理</span>
                    </td>
                </tr>
                </#list>
            </#if>
